"""Verify the complete V6 filter against frozen decisions, without audio inference."""
...
def verify(directory, output):
    ...
    items.extend(item for item in cached_external(directory,
                 directory.parent / 'relational-piano-training/manifest.json', directory)
                 if item['group'] == 'validation')
    items.extend(item for item in cached_external(directory,
                 directory.parent / 'left-hand-expanded-data/manifest.json', directory)
                 if item['group'] == 'validation')
    items.extend(cached_external(directory, directory.parent / 'relational-piano-fresh/manifest.json', directory))
    ...
    for item in prepared:
        ...
        low = {**item, 'keep': expected,
               'shared': item['shared'] & (item['events'][:, 2] >= 36) & (item['events'][:, 2] < 60)}
        expected = residual_keep(low, verifier.left_hand_model.probability(item['x']),
                                 verifier.left_hand_model.threshold)
        low['keep'] = expected
        expected = residual_keep(low, verifier.left_refinement_model.probability(item['x']),
                                 verifier.left_refinement_model.threshold)
    ...
    result = {'passes': True, 'recordings': len(items), 'candidate_events': total,
              'total_rejected': rejected, 'model_sha256': service.LEFT_REFINEMENT_SHA256,
